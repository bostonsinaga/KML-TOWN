    ...
    if (kmlNode) {
        ...
        if (mainFolderNode) {
            ...
            */
            ...
            /*
                not 'isFolderInsertion' means 'croppedPlacemarkNodes'
                are placemarks instead a folder (dualism)
            */
        }
        else isSucceeded = false;
    }
...
std::vector<xml::Node*> call_briefer::sortPlacemarksFunc(
    ...
    for (int i = 0; i < typeNamesVec.size(); i++) {
        ...
        if (placemarks.size() > 0) {
            ...
            std::cerr << "KML-> Sorting error. No cropped " << typeNamesVec.at(i) << "s as input\n";
        }
    }
    ...
    /*
        returns placemarks vector if 'isFolderInsertion' true
